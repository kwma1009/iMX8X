...
static int find_ir_chip_on_i2c(void)
{
	...
	for (i = 0; i < (ARRAY_SIZE(ir_i2c_addr) / sizeof(ir_i2c_addr[0])); i++) {
...
int adjust_vdd(ulong vdd_override)
{
	...
	if (!vid || vid == FSL_CHASSIS3_DCFG_FUSESR_ALTVID_MASK) {
...
int adjust_vdd(ulong vdd_override)
{
	...
	if (!vid || vid == FSL_CHASSIS2_DCFG_FUSESR_ALTVID_MASK) {
	...
	if (!vid || vid == FSL_CORENET_DCFG_FUSESR_ALTVID_MASK) {
