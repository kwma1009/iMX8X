...
static void mxc_pcsi_csr_config(struct mxc_parallel_csi_dev *pcsidev)
{
	...
	val |= CSI_CTRL_REG_DATA_TYPE_IN(pcsidev->input_type);

	if (pcsidev->invert_hsync)
		val |= CSI_CTRL_REG_HSYNC_POL;

	if (pcsidev->invert_vsync)
		val |= CSI_CTRL_REG_VSYNC_POL;
...
static int mxc_parallel_csi_probe(struct platform_device *pdev)
{
	...
	/*
	 * Note to Bayer users:  Set the input type to RGB 8 bit (type 2).
	 * Although the manual indicates Bayer as an option for the input,
	 * you cannot get raw Bayer data on the output with that setting.
	 */
	ret = of_property_read_u8(pdev->dev.of_node, "input-type",
				  &pcsidev->input_type);
	if (ret < 0) {
		dev_dbg(dev, "property input-type not specified\n");
		goto e_clkdis;
	}
	if (pcsidev->input_type > 0xf) {
		dev_dbg(dev, "Invalid setting for property input-type\n");
		goto e_clkdis;
	}

	if (of_property_read_bool(pdev->dev.of_node, "invert-hsync"))
		pcsidev->invert_hsync = 1;
	if (of_property_read_bool(pdev->dev.of_node, "invert-vsync"))
		pcsidev->invert_vsync = 1;
